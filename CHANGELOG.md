...
- **Breaking:** compaction is now ordinary, fallible turn work streamed like a tool call, split into three layers: `triggers` (when to ask), a new optional `shouldCompact` decision policy (whether — a `false` is the only silent path), and `compact` (the work — returns `AxleMessage[]`, never `null`; failures throw). The `CompactionPart` is stateful (`running`/`complete`/`error`), lands at the head of the send's turn for `beforeTurn`, the tail for `afterTurn`, or its own engine-opened turn for `manual`, and streams progressive summary text via the new `ctx.emit`. Automatic-trigger failures no longer reject the send — the errored part is the record and the conversation continues uncompacted.
...
- **Breaking:** `CompactionCallback` context loses `lastCompaction` and gains the engine-generated `id` and `emit`; `agent.compact()` returns `Promise<boolean>`; `CompactionRecord` is replaced by the `CompactionStamp` message-metadata convention (`axleCompaction: { id, role }`), which compactors use to mark their output and the engine uses to surface the summary on the settled part.
- `PromptCompactor` ships a bound `shouldCompact` (threshold/manual/empty policy), streams its summary through `ctx.emit`, stamps its output, and returns the summary and the recent-user-messages appendix as two separate messages; it recognizes its own prior output via the stamp, so consecutive compactions never re-quote an earlier summary.
