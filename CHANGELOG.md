...
- **Breaking:** removed the `AgentMemory`, `MemoryContext`, and `RecallResult` contracts, `AgentConfig.memory`, automatic recall/record behavior, and the CLI's `ProceduralMemory` export. Model-directed retrieval and writing should be ordinary tools; host-directed context can be attached with `Instruct.addContext()`, while transcript persistence remains an event-stream concern.
- **Breaking:** turns left the Agent. `agent.history` is removed; the active conversation is `agent.messages`, and transcripts are host-owned folds of the event stream (`TurnAccumulator` is renamed to `Transcript`, with `TranscriptInput` and `TranscriptApplyResult`). Attach one via `agent.on`; `transcript.turns` is the readonly read/persistence surface, and the constructor accepts and shallow-copies saved turns. Reader signatures should accept `readonly Turn[]`. `TranscriptState` and the redundant state on `apply()` results are removed. `AgentSession` is now the pure continuation `{ sessionId, messages }` — `version`, `archive`, `compactions`, `turns`, and `sessionAnnotations` are gone, and unknown keys in old stored sessions are ignored on restore.
- **Breaking:** session-level annotation targets and transcript state are removed. Annotations now belong to turns or parts; session-wide application state remains host-owned.
- **Breaking:** compaction is now ordinary, fallible turn work streamed like a tool call, split into three layers: `triggers` (when to ask automatically), a new optional synchronous `shouldCompactOnTrigger` policy (whether to accept an automatic request), and `compact` (the work — returns `{ messages, summary? }`, never `null`; failures throw). Explicit `agent.compact()` always runs the configured compactor. The `CompactionPart` is stateful (`running`/`complete`/`error`), lands at the head of the send's turn for `beforeTurn`, the tail for `afterTurn`, or its own engine-opened turn for `manual`, and accepts replacement `summary`/`progress` state via the new `ctx.emit`. Automatic-trigger failures no longer reject the send — the errored part is the record and the conversation continues uncompacted.
- **Breaking:** `compaction:start`/`compaction:end` and `session:restore` are deleted from the event protocol, replaced by a part-level lifecycle mirroring actions: `compaction:update`, `compaction:complete`, `compaction:error`.
- **Breaking:** `CompactionCallback` context loses `lastCompaction` and gains the engine-generated `id` and `emit`; `agent.compact()` returns `Promise<boolean>`; `CompactionRecord` is replaced by the `CompactionStamp` message-metadata convention (`axleCompaction: { id, role }`), which compactors use to recognize their own prior output; the part's reader-facing summary is returned directly from `compact` (`summary` — a presentation choice, independent of the model-facing messages).
- `PromptCompactor` ships a bound `shouldCompactOnTrigger` threshold/empty policy, reports estimated progress through `ctx.emit` without exposing generated summary tokens, stamps its output, and returns the summary and the recent-user-messages appendix as two separate messages plus the summary text as the part's reader-facing summary; it recognizes its own prior output via the stamp, so consecutive compactions never re-quote an earlier summary.
- Added a 0.30.0 migration guide.
