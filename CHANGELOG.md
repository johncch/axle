...
- **Breaking:** compaction is now ordinary, fallible turn work streamed like a tool call, split into three layers: `triggers` (when to ask), a new optional synchronous `shouldCompact` decision policy (whether — a `false` is the only silent path), and `compact` (the work — returns `{ messages, summary? }`, never `null`; failures throw). The `CompactionPart` is stateful (`running`/`complete`/`error`), lands at the head of the send's turn for `beforeTurn`, the tail for `afterTurn`, or its own engine-opened turn for `manual`, and accepts replacement `summary`/`progress` state via the new `ctx.emit`. Automatic-trigger failures no longer reject the send — the errored part is the record and the conversation continues uncompacted.
...
- `PromptCompactor` ships a bound `shouldCompact` (threshold/manual/empty policy), reports estimated progress through `ctx.emit` without exposing generated summary tokens, stamps its output, and returns the summary and the recent-user-messages appendix as two separate messages plus the summary text as the part's reader-facing summary; it recognizes its own prior output via the stamp, so consecutive compactions never re-quote an earlier summary.
