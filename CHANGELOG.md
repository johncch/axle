...
- **Breaking:** turns left the Agent. `agent.history` is removed; the active conversation is `agent.messages`, and transcripts are host-owned folds of the event stream (`TurnAccumulator` attached via `agent.on`, persisted and re-seeded by the host). `AgentSession` is now the pure continuation `{ sessionId, messages }` — `version`, `archive`, `compactions`, `turns`, and `sessionAnnotations` are gone, and unknown keys in old stored sessions are ignored on restore.
