...
- **Breaking:** compaction is now ordinary, fallible turn work streamed like a tool call, split into three layers: `triggers` (when to ask), a new optional `shouldCompact` decision policy (whether — a `false` is the only silent path), and `compact` (the work — returns `{ messages, summary? }`, never `null`; failures throw). The `CompactionPart` is stateful (`running`/`complete`/`error`), lands at the head of the send's turn for `beforeTurn`, the tail for `afterTurn`, or its own engine-opened turn for `manual`, and streams progressive summary text via the new `ctx.emit`. Automatic-trigger failures no longer reject the send — the errored part is the record and the conversation continues uncompacted.
