...
describe("TurnBuilder", () => {
  ...
  test("thinking streaming produces correct events", () => {
    ...
    events.push(...builder.handleStreamEvent({
      type: "thinking:delta",
      index: 0,
      delta: "Thinking...",
      accumulated: "Thinking...",
    }));
    events.push(...builder.handleStreamEvent({ type: "thinking:end", index: 0, final: "Thinking..." }));
  ...
  test("tool call lifecycle produces correct events", () => {
    ...
    // The turn should have an action part
    expect(turn.parts).toHaveLength(1);
